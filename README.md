...
6. **Agents & skills** → Pre-installed in `.github/agents/` and `.github/skills/`
7. **Copilot config** → Pre-installed in `.github/copilot-instructions.md` and `.github/copilot-setup-steps.yml`
...
│   ├── cobol/CUSTMGMT.cbl         #   Sample program (157 LOC)
...
│   ├── program-inventory.json      #   4 sample program entries
│   ├── copybook-catalog.json       #   1 copybook entry
│   ├── jcl-catalog.json            #   1 JCL job entry
│   └── data-dictionary.json        #   7 VSAM file entries
│   # Note: These counts are generated by scripts/scan-codebase.py (drift is self-explanatory). Run ./scripts/bootstrap.sh (or .ps1) to rebuild data/.
│
...
│   └── testing-prompts.md
│
├── custom-agents/                  # (Consolidated into .github/agents/)
├── custom-skills/                  # (Consolidated into .github/skills/)
└── copilot-config/                 # (Consolidated into .github/)
```
