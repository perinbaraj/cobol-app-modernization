...
public record ParityReport(
    ...
    public String summary() {
        return String.format("Parity: %d/%d (%.1f%%) - %s",
            passed, totalTests, passRate,
            isPassing() ? "PASS" : "FAIL");
    }
}
...
mvn test -Dgroups=parity -DreportFormat=html
```
...
- name: Upload Parity Report
  ...
  with:
    name: parity-report
    path: target/parity-reports/
```

## Exit Criteria
A migration is considered complete when:
1. All parity tests pass (100% match)
2. Edge cases are documented and accepted
3. Any intentional differences are documented with business approval
